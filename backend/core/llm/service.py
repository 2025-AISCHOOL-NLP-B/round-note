...
class LLMService:
    def __init__(self):
        # 필요한 초기화 로직 (예: OpenAI 클라이언트, 설정값 등)
        pass

    async def get_action_items_only(self, transcript_texts: list[str]):
        """
        전사 텍스트 기반으로 액션아이템만 추출
        """
        action_items = await self._call_llm_for_actions(transcript_texts)
        return {"action_items": action_items}

    async def get_translation(self, text: str, target_lang: str = "en"):
        """
        입력된 텍스트를 target_lang으로 번역
        """
        translation = await self._call_llm_for_translation(text, target_lang)
        return translation

    # 내부 LLM 호출 메서드들 (실제 구현은 OpenAI 등 API 호출)
    async def _call_llm_for_actions(self, transcript_texts: list[str]):
        """
        LLM API 호출 → 액션아이템 추출
        """
        # TODO: 실제 LLM API 호출 로직 작성
        # 아래는 예시 응답
        return [
            {"task": "산출 문서 마무리", "assignee": "권현재", "deadline": "2025-12-01"},
            {"task": "액션 아이템 생성 마무리", "assignee": "김기찬", "deadline": "2025-11-21"},
        ]

    async def _call_llm_for_translation(self, text: str, target_lang: str):
        """
        LLM API 호출 → 번역
        """
        # TODO: 번역 API 호출 로직 작성
